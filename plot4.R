...
png(file = "plot4.png", width = 480, height = 480)              ## plot to a 480x480 PNG file

## display 4 plots on a 2x2 matrix.
...
## on top left: plot the global active power wrt the time
plot(data$DateAndTime,                                          ## one X position per data row = per minute
     ...
     ylab="Global Active Power")                                ## the label along the Y axis


## on top right: plot the voltage wrt the time
plot(data$DateAndTime,                                          ## one X position per data row = per minute
     data$Voltage,                                              ## put Voltage along the Y axis
     "l",                                                       ## display as connected lines
     xlab = "datetime",                                         ## the label along the X axis
     ylab="Voltage")                                            ## the label along the Y axis


## on bottom left: plot the three sub-meterings wrt the time
...
## This time, we don"t want a box around it
legend("topright", bty = "n", lty = 1, col = colors, legend = names(data)[7:9])


## on bottom right: plot the global reactive power wrt the time
plot(data$DateAndTime,                                          ## one X position per data row = per minute
     data$Global_reactive_power,                                ## put Global reactive power along the Y axis
     "l",                                                       ## display as connected lines
     xlab = "datetime",                                         ## the label along the X axis
     ylab="Global_reactive_power")                              ## the label along the Y axis
